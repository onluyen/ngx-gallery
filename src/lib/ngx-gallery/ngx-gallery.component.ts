import { Component, OnInit, AfterViewInit, EventEmitter, Output, ViewChild, HostBinding, ElementRef, HostListener, input, signal, computed } from '@angular/core';
...
@Component({
    ...
    <div class="ngx-gallery-layout {{currentOptions()?.layout}}">
      ...
      @if (currentOptions()?.thumbnails) {
        <ngx-gallery-thumbnails [style.marginTop]="getThumbnailsMarginTop()" [style.marginBottom]="getThumbnailsMarginBottom()" [style.height]="getThumbnailsHeight()" [images]="smallImages()" [links]="currentOptions()?.thumbnailsAsLinks ? links() : []" [labels]="labels()" [linkTarget]="currentOptions()?.linkTarget" [selectedIndex]="selectedIndex()" [columns]="currentOptions()?.thumbnailsColumns" [rows]="currentOptions()?.thumbnailsRows" [margin]="currentOptions()?.thumbnailMargin" [arrows]="currentOptions()?.thumbnailsArrows" [arrowsAutoHide]="currentOptions()?.thumbnailsArrowsAutoHide" [arrowPrevIcon]="currentOptions()?.arrowPrevIcon" [arrowNextIcon]="currentOptions()?.arrowNextIcon" [clickable]="currentOptions()?.image || currentOptions()?.preview" [swipe]="currentOptions()?.thumbnailsSwipe" [size]="currentOptions()?.thumbnailSize" [moveSize]="currentOptions()?.thumbnailsMoveSize" [order]="currentOptions()?.thumbnailsOrder" [remainingCount]="currentOptions()?.thumbnailsRemainingCount" [lazyLoading]="currentOptions()?.lazyLoading" [actions]="currentOptions()?.thumbnailActions"  (onActiveChange)="selectFromThumbnails($event)"></ngx-gallery-thumbnails>
      }
...
export class NgxGalleryComponent implements OnInit, AfterViewInit {
    ...
    @Output() change = new EventEmitter<{ index: number; image: NgxGalleryImage | string; }>();
    @Output() previewOpen = new EventEmitter<void>();
    @Output() previewClose = new EventEmitter<void>();
    @Output() previewChange = new EventEmitter<{ index: number; image: NgxGalleryImage | string; }>();

    smallImages = computed<(string | SafeResourceUrl)[]>(() => (this.images() || []).map((img) => typeof img === 'string' ? img : <string>(img.small ?? img.medium ?? img.big)));
    mediumImages = computed<NgxGalleryOrderedImage[]>(() => (this.images() || []).map((img, i) => new NgxGalleryOrderedImage({ src: typeof img === 'string' ? img : <string>(img.medium ?? img.small ?? img.big), index: i })));
    bigImages = computed<(string | SafeResourceUrl)[]>(() => (this.images() || []).map((img) => typeof img === 'string' ? img : <string>(img.big ?? img.medium ?? img.small)));
    descriptions = computed<string[]>(() => (this.images() || []).map((img) => typeof img === 'string' ? '' : <string>(img.description)));
    links = computed<string[]>(() => (this.images() || []).map((img) => typeof img === 'string' ? '' : <string>(img.url)));
    labels = computed<string[]>(() => (this.images() || []).map((img) => typeof img === 'string' ? '' : <string>(img.label)));
    ...
    ngOnInit() {
        const opts = (this.options() || []).map((opt) => new NgxGalleryOptions(opt));
    ...
    private _imagesEffect = computed(() => {
        ...
        if (imgs && imgs.length) {
            ...
            const curr = this.currentOptions();
            if (curr && curr.thumbnailsAutoHide && curr.thumbnails && imgs.length <= 1) {
    ...
    getThumbnailsHeight(): string {
        const curr = this.currentOptions();
        if (curr && curr.image) {
            return 'calc(' + curr.thumbnailsPercent + '% - ' + curr.thumbnailsMargin + 'px)';
        } else {
            return '100%';
        }
    }
    ...
    selectFromThumbnails(index: number) {
        ...
        const curr = this.currentOptions();
        if (curr && curr.thumbnails && curr.preview && (!curr.image || curr.thumbnailsRemainingCount)) {
    ...
    canShowNext(): boolean {
        ...
        const curr = this.currentOptions();
        if (curr) {
            return (curr.imageInfinityMove || this.selectedIndex() < imgs.length - 1) ? true : false;
        } else {
            return false;
        }
    }
    ...
    private setBreakpoint(): void {
        ...
        if (typeof window !== 'undefined') {
            ...
            breakpoints = opts.filter((opt) => opt.breakpoint >= window.innerWidth)
